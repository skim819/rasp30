...
clear blk;
clear jlist cat_num;
clear loctmp loctmp2;
//LOAD OBJECT FILE

global file_name;
global jlist cat_num;
global board_num addvmm;
global plcvpr;
...
if chip_num == [] then
    messagebox('Please enter the FPAA board number that you are using.', "No FPAA Board Number Entered", "error");
    abort
end
...
iopad_loc(1,40).entries='13 0 1 #int[1]'; //west
...
for i =1:no
    ...
    elseif ( length(scs_m.objs(i) )==1) then
        ...
        elseif(scs_m.objs(i).gui== "pad_in") then
            //            if scs_m.objs(i).model.ipar(2) == 0 then 
            prime_ips(ip_count)=j;
            ip_count=ip_count+1;//primary ip count
            blk_name.entries(j)=  scs_m.objs(i).gui
            blk_objs(objnum)=j; //BLOCK NUMBER actually stored
        ...
        elseif   (scs_m.objs(i).gui== "pad_outd") then
            ...
            blk_objs(objnum)=j; //BLOCK NUMBER actually stored
            objnum=objnum+1;
            //        elseif(scs_m.objs(i).gui== "vdd") then
            //            fix_
            //            prime_ips(ip_count)=j;
            //            ip_count=ip_count+1;//primary ip count
            //            blk_name.entries(j)=  scs_m.objs(i).gui
            ...
            //        elseif   (scs_m.objs(i).gui== "IN_m") then
            //            blk_name.entries(j)=  strcat([scs_m.objs(i).gui ,scs_m.objs(i).graphics.exprs(1,1)])
            //            for count1=1: scs_m.objs(i).model.out2
            //                prime_ips(ip_count)=j;
            //                ip_count=ip_count+1;
            //                j=j+1;
            //            end
            //            //updates ip_count
            //            //ip_count=ip_count+size(scs_m.objs(i).model.opar,1);//primary ip count
            //            //OUT_M
            //        elseif   (scs_m.objs(i).gui== "OUT_m") then
            //            blk_name.entries(j)=  strcat([scs_m.objs(i).gui ,scs_m.objs(i).graphics.exprs(1,1)])
            //            for count1=1: scs_m.objs(i).model.in2
            //                prime_ips(op_count)=j;
            //                ip_count=op_count+1;
            //                j=j+1;
            //            end
            ...
            elseif (scs_m.objs(i).gui== "vdd_i") then
                fix_vdd = 1
                //addvcc = %t;
            elseif (scs_m.objs(i).gui== "vdd_o") then
                //fix_vdd = 2
            elseif (scs_m.objs(i).gui== "gnd_o") then
                //fix_gnd = 2
            ...
            elseif (scs_m.objs(i).gui== "generic_dig") then //suma added
                dig_blk = 1
            elseif (scs_m.objs(i).gui== "peak_det") then 
                //    addvcc = %t;
...
//jlist = cell([numoflink 2]);
//
//
//
//for m=1:numoflink
//    xx = scs_m.objs(link_blk(1,m)).from(1,1)
//    yy = scs_m.objs(link_blk(1,m)).to(1,1)
//    if (scs_m.objs(xx).gui == 'SPLIT_f') &  (scs_m.objs(yy).gui == 'SPLIT_f') then
//        if  jlist(xx,1).entries == [] then
//            jlist(xx,1).entries = yy
//        else
//            jlist(xx,2).entries = yy
//        end
//        junc_unq(1,1).entries = cat(2,junc_unq(1,1).entries,xx)
//    end
//end
//
//junc_unq(1,1).entries=unique(junc_unq(1,1).entries,2)
//junc_share = cell(length(junc_unq),1)
//if  junc_unq.entries ~= [] then
//
//    for m= 1:length(junc_unq)
//        if jlist(junc_unq(1,1).entries(m),1).entries ~= [] then
//            cat_num(1,1).entries=cat(2, cat_num(1,1).entries,junc_unq(1,1).entries(m))
//            junc_share(m,1).entries=junc(junc_unq(1,1).entries(m))
//            cat_num(1,1).entries = []
//        end
//    end
//
    ...
    for r=1:numofblk
        if(blk(r,1)==curblk) then
            ...
            if(scs_m.objs(curblk).gui=='SPLIT_f') then
                //                    if(spl_blk(curblk,1).entries == {}) then
                //                    disp('kl')
                //                    end
...
if add_gnd1 | add_gnd2 | fix_gnd | fix_vdd then
    global_string=".globals gnd vcc";
    //    if length(add_clk) ~= 0 then
    //        global_string=strcat([global_string,' neta']); 
    //    end
end

if add_gnd2 | (fix_vdd == 1) then
...
if add_gnd1 | (fix_gnd == 2) then
    prime_op_string=strcat([prime_op_string," gnd"]);
end

if fix_vdd == 2 then
    prime_op_string=strcat([prime_op_string," vdd"]);
end

if ramp_adc == 1 then
    prime_op_string=strcat([prime_op_string," out_ramp_adc"]);
end
...
if makeblk == %f then
    ...
    if add_gnd1 | add_gnd2 then
        mputl(global_string,fd_w);
    end
    mputl("  ",fd_w)
    mclose(fd_w);
    ...
    fd_io= mopen(fname+'.pads','wt')
    //////////mputl('dummy1 7 0 0 #tgate[0]',fd_io);
    //////////mputl('out:dummy2 7 0 3 #tgate[1]',fd_io);
    ////////////mputl('rst 8 0 0  #tgate[0]',fd_io);
    mclose(fd_io);
...
for bl=1:length(blk_objs)
    ...
    elseif (blk_name.entries(bl) =='c4_block') then
        ...
        for ss=1:scs_m.objs(bl).model.ipar(1)
            ...
            for ii=1:2
                ...
                select c4cap
                case 1 then
                    c4_str= c4_str +"&c4_cap_1x[" + string(ii-1) +"] =0"
                case 2 then
                    c4_str= c4_str +"&c4_cap_2x[" + string(ii-1) +"] =0"
                case 3 then
                    c4_str= c4_str +"&c4_cap_3x[" + string(ii-1) +"] =0"
                case 4 then
                    c4_str= c4_str +"&c4_cap_3x[" + string(ii-1) +"] =0"+"&c4_cap_1x[" + string(ii-1) +"] =0"
                case 5 then
                    c4_str= c4_str +"&c4_cap_3x[" + string(ii-1) +"] =0"+"&c4_cap_2x[" + string(ii-1) +"] =0"
                case 6 then
                    c4_str= c4_str +"&c4_cap_3x[" + string(ii-1) +"] =0"+"&c4_cap_2x[" + string(ii-1) +"] =0"+"&c4_cap_1x[" + string(ii-1) +"] =0"
                case 18 then
                    c4_str= c4_str +"&c4_cap_3x[0] =0"+"&c4_cap_2x[0] =0"+"&c4_cap_1x[0] =0"+"&c4_cap_3x[2] =0"+"&c4_cap_2x[2] =0"+"&c4_cap_1x[2] =0" +"&c4_cap_3x[3] =0"+"&c4_cap_2x[3] =0"+"&c4_cap_1x[3] =0"   
                else
                    error("C4 capacitor cannot be compiled.")
                end
            end
            mputl(c4_str,fd_w)
            mputl("  ",fd_w)
    ...
    elseif (blk_name.entries(bl) =='cap') then
        mputl("# CAP",fd_w);
        cap_i=1;
        cap_str = ".subckt cap in[0]=net"+string(blk(blk_objs(bl),2))+"_"+string(cap_i)+" out[0]=net"+ string(blk(blk_objs(bl),2+numofip))+"_" + string(cap_i);
        capcap = scs_m.objs(blk_objs(bl)).model.rpar(1)
        select capcap
        case 1 then
            cap_str= cap_str +" #cap_1x =0" 
        case 2 then
            cap_str= cap_str +" #cap_2x =0"
        case 3 then
            cap_str= cap_str +" #cap_3x =0"
        case 4 then
            cap_str= cap_str +" #cap_3x =0" + "&cap_1x =0" 
        case 5 then
            cap_str= cap_str +" #cap_3x =0" +"&cap_2x =0"
        case 6 then
            cap_str= cap_str +" #cap_3x =0"+"&cap_2x =0"+"&cap_1x =0" 
        else
            error("Capacitor cannot be compiled.")
        end
        mputl(cap_str,fd_w)
  ...
  //************************* Common Drain ******************************
    ...
    elseif(blk_name.entries(bl)=='dac_o')  then
        ...
        for ss=1:scs_m.objs(bl).model.ipar(1)
            dac_loc_idx = dac_loc_idx +1;
            dc(1,1).entries(1,ss)= strtod(dac_loc(loc_num,dac_loc_idx).entries(2))
            //dac_net(blk(blk_objs(bl),2),1).entries=dac_loc(dac_loc_idx,3).entries;
...
for bl=1:length(blk_objs)
    ...
    elseif(blk_name.entries(bl)=='gnd_o')  then
        if chgnet(2) == 0 then
            mputl("# gnd_out",fd_w);
            mputl(".subckt gnd_out in[0]=fb_gndout in[1]=gnd_out out[0]=fb_gndout #gnd_out_c =0",fd_w);
            mputl(' ',fd_w);
            chgnet(2)=1;
        end
        chgnet_dict=[chgnet_dict;'net'+ string(blk(blk_objs(bl),2))+"_.",'gnd_out']
        chgnet_tf=%t;
    ...
    elseif (blk_name.entries(bl) =='hhneuron') then 
        ...
        for ss = 1:scs_m.objs(bl).model.ipar(1)
            mputl(".subckt hhneuron in[0]=net"+string(blk(blk_objs(bl),2))+ "_" + string(ss)+" in[1]=net"+string(blk(blk_objs(bl),3))+ "_" + string(ss)+" in[2]=net"+string(blk(blk_objs(bl),4))+ "_" + string(ss)+" in[3]=net"+string(blk(blk_objs(bl),5))+ "_" + string(ss)+" out[0]=net"+ string(blk(blk_objs(bl),2+numofip))+ "_" + string(ss)+" out[1]=net"+ string(blk(blk_objs(bl),3+numofip))+ "_" + string(ss)+" out[2]=net"+ string(blk(blk_objs(bl),4+numofip))+ "_" + string(ss)+ " #hhneuron_fg[0] =0&hh_vinbias =" +string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(8*ss-7)))+"&hh_nabias =" +string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(8*ss-6)))+"&hh_kbias =" +string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(8*ss-5)))+"&hh_fbpfetbias =" +string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(8*ss-4)))+"&hh_voutbias =2e-6"+"&hh_ota_p_bias[0] =" +string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(8*ss-3)))+"&hh_ota_n_bias[0] =" +string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(8*ss-2)))+"&hh_ota_p_bias[1] =" +string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(8*ss-1)))+"&hh_ota_n_bias[1] =" +string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(8*ss))),fd_w);
            mputl("  ",fd_w)
        end
    ...
    elseif (blk_name.entries(bl) =='integrator') then
        ...
        for ss=1:ii
            ...
            select capcap
            case 1 then
                integrator_str= integrator_str +"&cap_1x =0" 
            case 2 then
                integrator_str= integrator_str +"&cap_2x =0"
            case 3 then
                integrator_str= integrator_str +"&cap_3x =0"
            case 4 then
                integrator_str= integrator_str +"&cap_3x =0" + "&cap_1x =0" 
            case 5 then
                integrator_str= integrator_str +"&cap_3x =0" +"&cap_2x =0"
            case 6 then
                integrator_str= integrator_str +"&cap_3x =0"+"&cap_2x =0"+"&cap_1x =0" 
            else
                error("Capacitor cannot be compiled.")
            end
    ...
    elseif(blk_name.entries(bl)=='pad_in')  then 
        ...
        for ss=1:scs_m.objs(bl).model.ipar(1)
            ...
            if tmp_pad(3) == '3'  | tmp_pad(3)== '5' then
                tmp_pad(3) = '1';
            elseif tmp_pad(3) == '2'  then
                tmp_pad(3) = '0';
            end 
            netout = 'net'
            if tmp_pad(4)=='#int[3]'|tmp_pad(4)=='#int[2]'|tmp_pad(4)=='#int[1]' then
    ...
    elseif (blk_name.entries(bl) =='lpf') then 
        ...
        for otabuf_i = 1:scs_m.objs(bl).model.rpar(1)
            ...
            select lpfcap
            case 1 then
                lpf_str= lpf_str +"&lpf_cap_1x[0] =0"
            case 2 then
                lpf_str= lpf_str +"&lpf_cap_2x[0] =0"
            case 3 then
                lpf_str= lpf_str +"&lpf_cap_3x[0] =0"
            case 4 then
                lpf_str= lpf_str +"&lpf_cap_3x[0] =0"+"&lpf_cap_1x[1] =0"
            case 5 then
                lpf_str= lpf_str +"&lpf_cap_3x[0] =0"+"&lpf_cap_2x[1] =0"
            case 6 then
                lpf_str= lpf_str +"&lpf_cap_3x[0] =0"+"&lpf_cap_2x[1] =0"+"&lpf_cap_1x[2] =0"
            case 18 then
                lpf_str= lpf_str +"&lpf_cap_3x[0] =0"+"&lpf_cap_2x[0] =0"+"&lpf_cap_1x[0] =0"+"&lpf_cap_3x[2] =0"+"&lpf_cap_2x[2] =0"+"&lpf_cap_1x[2] =0" +"&lpf_cap_3x[3] =0"+"&lpf_cap_2x[3] =0"+"&lpf_cap_1x[3] =0"   
            else
                error("LPF capacitor cannot be compiled.")
            end
            mputl(lpf_str,fd_w)            
            mputl("  ",fd_w)
        end
    ...
    elseif (blk_name.entries(bl) =='meas_volt') then
        mputl("# MEAS_VOLT_2MITE*",fd_w);
        for meas = 1:scs_m.objs(bl).model.rpar(2)
            mputl(".subckt meas_volt_mite in[0]=net"+string(blk(blk_objs(bl),2))+'_'+ string(meas)+' out[0]=gnd #meas_fg =0.00001',fd_w);
            mputl("  ",fd_w)
        end
    ...
    elseif (blk_name.entries(bl) =='mmap_local_swc') then
        ...
        //****************************** MITE***********************************
    elseif (blk_name.entries(bl) =='mite') then
        mputl("# MITE",fd_w);
        mputl(".subckt mite in[0]=net"+string(blk(blk_objs(bl),2))+ " in[1]=net"+string(blk(blk_objs(bl),3))+ " in[2]=net"+string(blk(blk_objs(bl),4))+" out=net"+ string(blk(blk_objs(bl),2+numofip)),fd_w);
        mputl("  ",fd_w)
    ...
    elseif (blk_name.entries(bl) =='mite2_output') then
        ...
        //            output_compile(scs_m.objs(bl).model.rpar(1),scs_m.objs(bl).model.rpar(1));
        output_compile(1,1);
    ...
    elseif (blk_name.entries(bl) =='nfet') then
        mputl("# NFET",fd_w);
        mputl(".subckt nfet in[0]=net"+string(blk(blk_objs(bl),2))+'_1'+ " in[1]=net" + string(blk(blk_objs(bl),3)) +'_1'+ " out[0]=net"+ string(blk(blk_objs(bl),2+numofip))+'_1',fd_w);
        mputl("  ",fd_w)
        //        fd_info= mopen('info.txt','a+')
        //        mputl('Nfet gate=net"+string(blk(blk_objs(bl),2))+" drain=net" + string(blk(blk_objs(bl),3))',fd_info);
        //        mclose(fd_info);
    ...
    elseif (blk_name.entries(bl) =='peakdet_block') then
        ...
        for ss=1:scs_m.objs(bl).model.ipar(1)
            ...
            select capcap
            case 1 then
                cap_str= cap_str +"&c4_cap_1x[0] =0" 
            case 2 then
                cap_str= cap_str +"&c4_cap_2x[0] =0"
            case 3 then
                cap_str= cap_str +"&c4_cap_3x[0] =0"
            case 4 then
                cap_str= cap_str +"&c4_cap_3x[0] =0" + "&c4_cap_1x[0] =0" 
            case 5 then
                cap_str= cap_str +"&c4_cap_3x[0] =0" +"&c4_cap_2x[0] =0"
            case 6 then
                cap_str= cap_str +"&c4_cap_3x[0] =0"+"&c4_cap_2x[0] =0"+"&c4_cap_1x[0] =0" 
            else
                error("Capacitor for Peak Detector cannot be compiled.")
            end
            mputl(cap_str,fd_w)
            mputl("  ",fd_w)
        end
    ...
    elseif (blk_name.entries(bl) =='Ramp_ADC') then
        ...
        select board_num
        case 2 then
        mputl("out:out_ramp_adc 15 1 5 #int[5]",fd_io);
        case 3 then 
        mputl("out:out_ramp_adc 1 15 3 #int[3]",fd_io);
        end
        mclose(fd_io); 
...
//out[1]=net"+string(blk(blk_objs(bl),3+numofip))+"_1"+" out[2]=net"+string(blk(blk_objs(bl),4+numofip))+"_1"+" out[3]=net"+string(blk(blk_objs(bl),5+numofip))+"_1"+" 
       plcloc=[plcloc;"net"+string(blk(blk_objs(bl),2+numofip))+"_1",' 10 7 0'];
 mputl(sr_1i_16o_str,fd_w)
        mputl("  ",fd_w)
 end
       //global sftreg_check
    ...
    elseif (blk_name.entries(bl) =='speech') then
        ...
        for ss=1:scs_m.objs(bl).model.ipar(1)
            ...
            speech_str= '.subckt speech in[0]=net' + string(blk(blk_objs(bl),2))+"_1 in[1]=net"+string(blk(blk_objs(bl),3))+"_1 in[2]=vcc out[0]=net'+ string(blk(blk_objs(bl),2+numofip))+"_" + string(ss)+" #c4_ota_bias[0] =" +string(sprintf('%1.3e',scs_m.objs(blk_objs(bl)).model.rpar(2*ss-1)))+"&c4_ota_bias[1] =" +string(sprintf('%1.3e',scs_m.objs(blk_objs(bl)).model.rpar(2*ss)))+"&speech_fg[0] =0&c4_ota_p_bias[0] =100e-9&c4_ota_n_bias[0] =100e-9&c4_ota_p_bias[1] =100e-9&c4_ota_n_bias[1] =100e-9&speech_peakotabias[0] =100e-9&speech_pfetbias[0] =5e-11&speech_peakotabias[1] =200e-9&speech_pfetbias[1] =150e-9";
            mputl(speech_str,fd_w)
            mputl("  ",fd_w)

            plcloc=[plcloc;'net'+string(blk(blk_objs(bl),2+numofip))+'_'+ string(ss),' 2 '+string(ss)+' 0'];
        end
    ...
    elseif (blk_name.entries(bl) =='tgate') then
        mputl("# TGATE",fd_w);
        mputl(".subckt tgate in[0]=net"+string(blk(blk_objs(bl),2))+'_1' +" in[1]=net" + string(blk(blk_objs(bl),3)) + '_1' +" out=net"+ string(blk(blk_objs(bl),2+numofip))+'_1',fd_w);
        mputl("  ",fd_w)
        //        fd_info= mopen('info.txt','a+')
        //        mputl('Tgate sel=net"+string(blk(blk_objs(bl),2))+" in=net" + string(blk(blk_objs(bl),3))',fd_info);
        //        mclose(fd_info);
        if scs_m.objs(bl).model.rpar(1) == 1 then
            plcvpr = %t;
            plcloc=[plcloc;'net'+string(blk(blk_objs(bl),2+numofip))+'_1',' '+string(scs_m.objs(bl).model.rpar(2))+' '+string(scs_m.objs(bl).model.rpar(3))+' 0'];
    ...
    elseif(blk_name.entries(bl)=='vdd_o')  then
        if chgnet(5) == 0 then
            mputl("# vdd_out",fd_w);
            mputl(".subckt vdd_out in[0]=fb_vddout in[1]=vdd_out out[0]=fb_vddout #vdd_out_c =0",fd_w);
            mputl(' ',fd_w);
            chgnet(5)=1;
        end
        chgnet_dict=[chgnet_dict;'net'+ string(blk(blk_objs(bl),2))+"_.",'vdd_out']
        chgnet_tf=%t;
    ...
    elseif (blk_name.entries(bl) =='vmm_8by4') then 
        ...
        //for ss=1:scs_m.objs(bl).model.ipar(1)
        //plcloc=[plcloc;'net'+string(blk(blk_objs(bl),2))+'_out',' 10 '+string(ss)+' 0'];
        // end
    end
end         // for loop was divided by Sihwan because the big for loop generated a error when we compiled.
...
for bl=1:length(blk_objs)
    ...
    elseif (blk_name.entries(bl) =='vmmwta') then 
        ...
        for i = 1:size(k,1)
            vmm12_1o = vmm12_1 + " in[12]=vw_nfetd" +" out[0]=net"+ string(blk(blk_objs(bl),2+numofip))+"_" + string(i)+" #vmm12x1_fg =0&vmm12x1_pfetbias ="+string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(6+i)))+ "&vmm12x1_otabias ="+string(sprintf('%1.2e',scs_m.objs(blk_objs(bl)).model.rpar(6)))
    ...
    elseif (blk_name.entries(bl) =='wta') then
        ...
        //     else
        //        blkname =  blk_name.entries(bl)
        //        execstr(blkname+'()')
        //        disp('here')
        //        disp("One of the blocks you are trying to compile is not in supported by RASP Tools")
    end//BLOCK_NAME COMPARISON
end;//for
...
if chgnet_tf == %t then
    retool(chgnet_dict,path,fname,ext)
end
