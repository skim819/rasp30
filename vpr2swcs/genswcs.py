...
# run through similar switch generation as normal lut/ff
			...
			sub_outputs=subblock.findall("outputs")[0].findall("port")[0].text.split()
		    if nsb.name in ["mite2[0]", "mult[0]","ladder_filter[0]","hhneuron[0]","hh_neuron_b_debug[0]","dendiff[0]",'speech[0]']:
			sub_outputs=subblock.findall("outputs")[0].findall("port")[0].text.split()
			#pdb.set_trace()
			...
			#pdb.set_trace()
                        if from_sub_name ==['SR4[0]']:
                            ...
                            try:
                                ...
                                pdb.set_trace()
			elif from_sub_name in ['mite2[0]','mult[0]','ladder_filter[0]','hh_neuron_b_debug[0]','dendiff[0]','hhneuron[0]','speech[0]']:
			    #pdb.set_trace()
                            try:
                                #if i<2:
...
def parseBlif(file_name):
    ...
    for line in f:
		...
		if subckt in ['lpf_2','meas_volt_mite','ramp_fe','cap','ota_buf','ota_buffer','lpf','nfet_i2v','pfet_i2v','HOP_bif','nmirror','wta_primary','common_source','common_source1','fg_io','nmirror_vmm']: ## single i/ps
                	key = ports[1] #this key is just output port name ## for all cases with a single i/p- o/p
		elif subckt in ['INFneuron','DAC_sftreg','latch_custom','integrator','integrator_nmirror','sigma_delta_fe','sftreg2','mmap_local_swc','speech','in2in_x1']: #3 i/ps
			key=ports[3]
			 #pdb.set_trace()
		elif subckt in ['gnd_out','vdd_out']: #2 i/ps
			key=ports[2]
		elif subckt in ['vmm_senseamp2','SR4','vmm4x4','mult','hhneuron','hh_neuron_b_debug']: #4 i/ps
			key=ports[4]
			...
			key=ports[7]
		elif subckt in ['vmm4x4_SR2','tgate_so','th_logic']: #8 i/ps
			key=ports[8]
