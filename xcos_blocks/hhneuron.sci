function [x,y,typ]=hhneuron(job,arg1,arg2)
    ...
    case 'set' then
        ...
        model=arg1.model;
        while %t do
            [ok,in_out_num,vinbias,nabias,kbias,pfetbias,nabiasp,nabiasn,kbiasp,kbiasn,exprs]=getvalue('Set HH Neuron Parameters',['Number of HH Neuron blocks';'Vin Bias';'Na Bias';'K Bias';'FG pfet bias';'Na bias p';'Na bias n';'K bias p';'K bias n'],list('vec',1,'vec',-1,'vec',-1,'vec',-1,'vec',-1,'vec',-1,'vec',-1,'vec',-1,'vec',-1),exprs)
            ...
            if ok then
                ...
                model.rpar = [vinbias;nabias;kbias;pfetbias;nabiasp;nabiasn;kbiasp;kbiasn]
    ...
    case 'define' then
        ...
        exprs=[sci2exp(in_out_num); sci2exp(vinbias) ; sci2exp(nabias) ; sci2exp(kbias) ; sci2exp(pfetbias) ; sci2exp(nabiasp) ; sci2exp(nabiasn) ; sci2exp(kbiasp) ; sci2exp(kbiasn)]
