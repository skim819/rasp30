...
if n_target_aboveVt_swc ~= 0 then
    ...
    j=1; fd = mopen('data'+info_name+'2.hex','r'); clear data_01; data_01 = [1:m_graph]; 
    str_temp = mgetstr(7,fd);
    while str_temp ~= "0xffff ",
        data_01(j,1) = msscanf(str_temp,"%x");
        for i=2:m_graph
            data_01(j,i) = msscanf(mgetstr(7,fd),"%x");
        end
        str_temp = mgetstr(7,fd);
        j=j+1;
    end
    mclose(fd); data_01(:,2)=data_01(:,2)*1e-5; //fprintfMat("Scurve_at_Vg3.6V_swc.data", data_01, "%5.15f");

    for i=3:m_graph
        data_01(:,i)=exp(polyval(p1,data_01(:,i),S1))/kapa_constant_or_not;
    end
...
if n_target_aboveVt_ota ~= 0 then
    ...
    mclose(fd); data_02(:,2)=data_02(:,2)*1e-5; //fprintfMat("Scurve_at_Vg3.6V_oota.data", data_02, "%5.15f");

    for i=3:m_graph
        data_02(:,i)=exp(polyval(p1,data_02(:,i),S1))/kapa_constant_or_not;
    end
...
if n_target_aboveVt_otaref ~= 0 then
    ...
    j=1; fd = mopen('data'+info_name+'2.hex','r'); clear data_03; data_03 = [1:m_graph]; 
    str_temp = mgetstr(7,fd);
    while str_temp ~= "0xffff ",
    ...
    mclose(fd); data_03(:,2)=data_03(:,2)*1e-5; //fprintfMat("Scurve_at_Vg3.6V_otaref.data", data_03, "%5.15f");

    for i=3:m_graph
        data_03(:,i)=exp(polyval(p1,data_03(:,i),S1))/kapa_constant_or_not;
    end
...
if n_target_aboveVt_mite ~= 0 then
    ...
    j=1; fd = mopen('data'+info_name+'2.hex','r'); clear data_04; data_04 = [1:m_graph]; 
    str_temp = mgetstr(7,fd);
    while str_temp ~= "0xffff ",
    ...
    mclose(fd); data_04(:,2)=data_04(:,2)*1e-5; //fprintfMat("Scurve_at_Vg3.6V_mite.data", data_04, "%5.15f");

    for i=3:m_graph
        data_04(:,i)=exp(polyval(p1,data_04(:,i),S1))/kapa_constant_or_not;
    end
...
if n_target_aboveVt_dirswc ~= 0 then
    ...
    j=1; fd = mopen('data'+info_name+'2.hex','r'); clear data_05; data_05 = [1:m_graph]; 
    str_temp = mgetstr(7,fd);
    while str_temp ~= "0xffff ",
    ...
    mclose(fd); data_05(:,2)=data_05(:,2)*1e-5; fprintfMat("Scurve_at_Vg3.6V_dirswc.data", data_05, "%5.15f");

    for i=3:m_graph
        data_05(:,i)=exp(polyval(p1,data_05(:,i),S1))/kapa_constant_or_not;
    end
