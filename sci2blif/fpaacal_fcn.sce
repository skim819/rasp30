...
function dir_callback() 
    ...
    if b == 1 then
        messagebox("Install Adobe Reader via the Documents menu on main RASP Design gui. ", "Adode Reader not installed yet!", "scilab")
    end  
endfunction
...
function Choose_Board2_callback()
    ...
    elseif (Choose_Board2.value == 2) then
        board_num=2;
        brdtype = '';
        disp('You are now using the settings for the 3.0 Board');
    elseif (Choose_Board2.value == 3) then
        board_num=3;
        brdtype = '_30a';
        disp('You are now using the settings for the 3.0 A Board');
    elseif (Choose_Board2.value == 4) then
        board_num=4;
        brdtype = '_30n';
        disp('You are now using the settings for the 3.0 N Board');
    elseif (Choose_Board2.value == 5) then
        board_num=5;
        brdtype = '_30h';
        disp('You are now using the settings for the 3.0 H Board');
    else
...
function Initate_Amm_callback()
    ...
    if amm_con1 then
        ...
        disp("FPAA Board has been initiated.")
    end
endfunction
...
function Step_1_callback()
    ...
    legend(" Data1","Polynomial Model1","Data2","Polynomial Model2","in_upper_left");xtitle("","ADC data","Ichar(A)");
    hex_1na = Current_to_ADC(5,2); // Hex value for current 1nA
    disp(hex_1na);
...
function Step_4_callback()
    ...
    exec("~/rasp30/prog_assembly/libs/scilab_code/characterization/char_diodeADC.sce",-1); // Call Diode ADC polifit function
    hex_1na = Current_to_ADC(5,2); // Hex value for current 1nA
...
function Step_5_callback()
    ...
    unix_w("cp ~/rasp30/prog_assembly/libs/scilab_code/characterization/char_diodeADC/data_diodeADC_chip"+chip_num+brdtype+"_ivdd25V data_diodeADC_ivdd25V');
    unix_w("cp ~/rasp30/prog_assembly/libs/scilab_code/characterization/char_diodeADC.sce char_diodeADC.sce");
    exec("char_diodeADC.sce",-1); // Call Diode ADC polifit function
    hex_1na = Current_to_ADC(5,2); // Hex value for current 1nA
...
function Step_6_callback()
    global file_name path fname extension chip_num board_num hex_1nA hex_1na showprog;
    showprog = 1;
